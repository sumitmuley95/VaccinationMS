...
if ($query) {
  // Fetch test details and patient information
  $testQuery = mysqli_query($con, "SELECT * FROM tbltestrecord WHERE id='$testid'");
  $testData = mysqli_fetch_array($testQuery);
  
  // Fetch the patient details from the tblpatients table using PatientMobileNumber
  $patientQuery = mysqli_query($con, "SELECT * FROM tblpatients WHERE MobileNumber='{$testData['PatientMobileNumber']}'");
  ...
  try {
      ...
      $mail->SMTPAuth = true;
      $mail->Username = 'macqueencars95@gmail.com';  // Your email address
      $mail->Password = 'exul unyc zviw becb';  // Your email password
      $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;
      $mail->Port = 465;

      // Recipients
      $mail->setFrom('macqueencars95@gmail.com', 'Hospital Management');
      $mail->addAddress($patientData['Email'], $patientData['FullName']); // Patient's email and name

      // Content
      $mail->isHTML(true);
      $mail->Subject = 'Test Assigned to Hospital';

      // Create the email body
      $mail->Body = "<h2>Test Assigned</h2>";
      $mail->Body .= "<p><strong>Patient Name:</strong> " . $patientData['FullName'] . "</p>";
      $mail->Body .= "<p><strong>Mobile Number:</strong> " . $patientData['MobileNumber'] . "</p>";
      $mail->Body .= "<p><strong>Test Type:</strong> " . $testData['TestType'] . "</p>";
      $mail->Body .= "<p><strong>Time Slot:</strong> " . $testData['TestTimeSlot'] . "</p>";
      $mail->Body .= "<p><strong>Assigned Hospital:</strong> " . $aname . "</p>";
      $mail->Body .= "<p><strong>Order Number:</strong> " . $testData['OrderNumber'] . "</p>";

      // Send the email
      $mail->send();
      echo '<script>alert("Assigned Hospital successfully and email sent.")</script>';
  ...
  echo '<script>alert("Failed to assign hospital.")</script>';
}
}
...
<head>
    ...
    <title>Vaccination Management System | Test Details</title>
...
if($rstatus=='Assigned' || $rstatus=='Vaccination Completed' || $rstatus=='Sample Collected' || $rstatus=='Sent to Lab'):?>
...
endwhile;
  ?>
            ...
            <option value="Sample Collected">Vaccination Remaining</option>
            <?php elseif($reportstatus=='Vaccination Completed'):?>
            <option value="Delivered">Upload Report</option>
